...
def T0(d):  # noqa
    ...
    d['vwap'] = np.where(d['v']!=0, d['a'] / d['v'], d['midp']) # 合理??
    d['vwap'] = d['vwap'].ffill()
    ...
    d['aa'] = np.sum(d[f'av{n}'] * d[f'av{n}']for n in levels) # ask amount
    d['ba'] = np.sum(d[f'av{n}'] * d[f'av{n}']for n in levels) # bid amount
